<script>
  ...
  const views = [
    { 
      name: 'India', 
      // Instead of viewBox, use transform values (x, y, scale)
    ...
    { 
      name: 'Punjab+Haryana', 
      // Convert viewBox [900, 1150, 1958, 1100] to transform values
      transform: { x: -900, y: -1150, scale: 3.0 },
      labelVisible: true,
      adjustments: {
    ...
    { 
      name: 'Uttar Pradesh', 
      transform: { x: -1800, y: -1800, scale: 3.0 },
      labelVisible: true,
      adjustments: {
        mobile: { offsetX: 90, offsetY: 100, scale: 0.8 },
        tablet: { offsetX: 150, offsetY: 0, scale: 1.0 },
    ...
    { 
      name: 'Bihar', 
      transform: { x: -3100, y: -2200, scale: 4.8 },
      labelVisible: true,
      adjustments: {
        mobile: { offsetX: 100, offsetY: -100, scale: 0.7 },
        tablet: { offsetX: 0, offsetY: 0, scale: 1.0 },
        desktop: { offsetX: 0, offsetY: 0, scale: 1.0 }
      },
      info: {
    ...
    { 
      name: 'West Bengal', 
      transform: { x: -3300, y: -2800, scale: 4.8 },
      labelVisible: true,
      adjustments: {
        mobile: { offsetX: -80, offsetY: 150, scale: 0.9 },
        tablet: { offsetX: 0, offsetY: 0, scale: 1.0 },
        desktop: { offsetX: 0, offsetY: 180, scale: 1.0 }
      },
      info: {
        title: "West Bengal",
    ...
    { 
      name: 'Assam', 
      transform: { x: -4200, y: -2250, scale: 4.8 },
      labelVisible: true,
      adjustments: {
        mobile: { offsetX: -120, offsetY: 150, scale: 0.7 },
        tablet: { offsetX: 0, offsetY: 0, scale: 1.0 },
        desktop: { offsetX: 0, offsetY: 180, scale: 1.0 }
      },
      info: {
  ...
  const DURATION = 800; // Consistent duration for all devices
  
  // Use CSS transforms instead of viewBox
  const transformStore = tweened(views[0].transform, {
    duration: DURATION,
    easing: cubicOut
  });
  ...
  transformStore.subscribe(value => {
  ...
  function updateSVGTransform() {
    ...
    // Apply transform to the SVG container instead of changing viewBox
    svgContainer.style.transform = `translate(${x}px, ${y}px) scale(${scale})`;
    svgContainer.style.transformOrigin = '0 0';
  }
  
  function getResponsiveTransform(baseTransform, windowWidth, windowHeight, step) {
    ...
    let responsiveScale = baseTransform.scale;
    
    if (windowWidth <= 480) {
      responsiveScale *= 1.5; // Zoom in more on small screens
    } else if (windowWidth <= 768) {
      responsiveScale *= 1.2;
    }
    ...
    // Apply adjustments
    return {
      x: baseTransform.x + adjustments.offsetX,
      y: baseTransform.y + adjustments.offsetY,
      scale: responsiveScale * adjustments.scale
    };
  }
  
  function updateTransform(step, width, height) {
    const baseTransform = views[step].transform;
    const responsiveTransform = getResponsiveTransform(baseTransform, width, height, step);
    transformStore.set(responsiveTransform);
  }
  ...
  function updateScrollPosition() {
    ...
    if (scrollY < totalScrollytellingHeight) {
      ...
      if (newStep !== currentStep) {
        currentStep = newStep;
        updateTransform(currentStep, window.innerWidth, window.innerHeight);
      }
  ...
  $: if (browser && innerWidth && innerHeight) {
    ...
    updateTransform(currentStep, innerWidth, innerHeight);
  }
...
<style>
  ...
  .svg-container {
    width: 6000px; /* Match your original SVG width */
    height: 6750px; /* Match your original SVG height */
...
<div class="svg-wrapper" class:hidden={!isScrollytellingActive}>
  <div class="svg-container" bind:this={svgContainer}>
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 6000 6750" preserveAspectRatio="xMidYMid meet">
